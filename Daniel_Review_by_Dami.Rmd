...
setwd("C:\\Users\\dabir\\OneDrive - Iowa State University\\Documents\\Dabiri\\output_folder")
```



```{R}
#This is one way to copy data into R. Another simple way is to copy directly from github, especailly if the reviewer doesnt have the data locally. For example https://raw.githubusercontent.com/EEOB-BioData/BCB546_Spring2024/main/assignments/UNIX_Assignment/snp_position.txt"
...
snp_data <- read.table(url(snp_position_url), header = TRUE, sep = "\t")
fang_data <- read.table(url(fang_et_al_genotypes_url), header = TRUE, sep = "\t")
```
```{r}
...
library(tidyverse)
# Constantly forcing viewing the data generated might not be a good way to follow up with the script.Preferably, having a peep of the data on the global envrionment will be better
...
#This would have been better done using the for command, this will iterate the comand and make it faster. 
...
```{r}
#using the 'write_tsv' command coupled with the for command will help create the file and also save them in the desired folder

#save all data to files
#create folder maize_data and teosinte_data
dir.create(file.path("C:\\Users\\dabir\\OneDrive - Iowa State University\\Documents\\teosinte_data"))
save(teosinte_chr1_data, file = "C:\\Users\\dabir\\OneDrive - Iowa State University\\Documents\\Daniel\\teosinte_chr1_data.Rdata")
...
#I learned alot from this script, new way to carry out data wrangling and data analysis.To help make your code readable and a proper workflow, I will suggest you put your lines of code in chunks and provide concise information about what each line of code does.
```
